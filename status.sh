...
#
# --- FIX: Persistent Client Name Retrieval (Added DB lookup to get_client_name) ---
...
get_client_name() {
    ...
    # 0. *** FIX START: Check ARCHIVE_DB_FILE for a historical name first for persistence ***
    # Look for the latest name associated with this MAC, but ignore entries where the name IS the MAC.
    name=$(sqlite3 "$ARCHIVE_DB_FILE" "SELECT name FROM daily_usage WHERE mac = '$clean_mac' AND name != '$clean_mac' ORDER BY date DESC LIMIT 1" | head -n 1)
    if [ -n "$name" ]; then
        final_name=$(echo "$name" | sed 's/^[ \t]*//;s/[ \t]*$//')
    fi
    # *** FIX END ***
    
    # 1. Check custom_clientlist (High Priority) - Only if no historical name was found
    ...
    # 2. Check static DHCP list - Only if no name found yet
    if [ -z "$final_name" ]; then
    ...
    # 3. Check current dnsmasq leases (Most Volatile) - Only if no name found yet
    if [ -z "$final_name" ]; then
        name=$(grep -i "$clean_mac" /var/lib/misc/dnsmasq.leases | awk '{print $4}' | head -n 1)
        if [ -n "$name" ] && [ "$name" != "*" ]; then final_name=$(echo "$name" | sed 's/^[ \t]*//;s/[ \t]*$//'); fi
    fi
    
    # 4. Attempt to parse raw data (if available) - Only if no name found yet
    if [ -z "$final_name" ]; then
...
        # NOTE: get_client_name now correctly prioritizes the historical name if current name is the MAC.
...
YEARLY_USAGE_DECIMAL=$(convert_usage $YEARLY_VALUE $YEARY_UNIT)
